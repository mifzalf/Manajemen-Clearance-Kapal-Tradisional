...
const ActionDropdown = ({ item, onEdit, onSuccess }) => {
  ...
  const onDelete = async () => {
    setIsOpen(!isOpen)
    if(confirm(`Hapus item ${item.nama_negara}?`)) {
      let response = await axios.delete(`${API_URL}/negara/delete/${item.id_negara}`)
...
const NegaraTable = ({ data = [], onEdit, onSuccess }) => {
  return (
      ...
      <table className="min-w-full divide-y divide-gray-200">
        <thead className="bg-gray-50">
          <tr>
            ...
            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Kode Negara</th>
            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nama Negara</th>
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {data.map((item, index) => (
            <tr key={item.id}>
              ...
              <td className="px-6 py-4 text-sm text-gray-500">{item.kode_negara}</td>
              <td className="px-6 py-4 text-sm text-gray-500">{item.nama_negara}</td>
