...
const KabupatenTable = ({ data = [], onEdit, provinsiList = [] }) => {
  const getProvinsiName = (provinsiId) => {
    const provinsi = provinsiList.find(p => p.id === provinsiId);
    return provinsi ? provinsi.nama : 'Tidak diketahui';
  };
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {data.map((item, index) => (
            <tr key={item.id}>
              ...
              <td className="px-6 py-4 text-sm text-gray-500">{getProvinsiName(item.provinsiId)}</td>
