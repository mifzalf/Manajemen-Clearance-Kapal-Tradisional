...
function Clearance() {
  ...
  const fetchFilteredData = useCallback(
    debounce(async (currentFilters) => {
      ...
      if (isFilterEmpty) {
        await fetchInitialDataAndOptions();
        return;
      }
      ...
      try {
        let params = {
          ...
          nama_muatan:
            currentFilters.selectedGoods.length > 0
              ? currentFilters.selectedGoods[0].value
              : '',
        };
  ...
  const exportXLSX = () => {
    ...
    XLSX.writeFile(
      workbook,
      `laporan_clearance_${new Date().toISOString().slice(0, 10)}.xlsx`
    );
    setIsExportOpen(false);
  };
  ...
  const selectedRowsPerPage = rowsPerPageOptions.includes(String(rowsPerPage))
    ? String(rowsPerPage)
    : 'Semua';
                ...
                {isExportOpen && (
                  <div className="absolute right-0 top-full mt-2 w-48 bg-white rounded-lg shadow-lg border z-20">
                    <ul className="p-1">
                      <li
                        onClick={exportXLSX}
                        className="rounded-md px-3 py-2 text-sm text-gray-700 hover:bg-gray-100 cursor-pointer"
                      >
              ...
              <div className="lg:col-span-4">
                <Select
                  ...
                  onChange={(selectedOptions) =>
                    handleFilterChange('selectedGoods', selectedOptions || [])
                  }
                  styles={customStyles}
                />
              </div>
